...
const TABS: Tab[] = [
  ...
  { id: "settings", label: "Institution Settings", icon: <FaCog /> },
];

const EditInstitutionPage: React.FC = () => {
  const navigate = useNavigate();
  const { instId } = useParams<{ instId: string }>();
  const { data: response, isLoading, error } = useInstitutionDetails();
      ...
      <div className="animate-in fade-in zoom-in-95 flex h-[80vh] flex-col items-center justify-center gap-6 duration-500">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 font-medium text-gray-500">
            The institution you're trying to manage doesn't exist or has been
            removed.
          </p>
        </div>
        <button
  ...
  // TODO: Add permission check (only owner/admin can access)
        ...
        <div className="mx-auto max-w-4xl px-4">
          ...
          <div className="flex items-center justify-between border-b border-gray-100 py-5">
            <div className="flex items-center gap-4">
              <button
                onClick={() => navigate(`/institutions/${instId}`)}
        ...
        <div className="flex flex-col gap-8">
          {activeTab === "photos" && (
            <div className="rounded-xl bg-white p-6 shadow-sm">
              <h2 className="text-lg font-bold text-gray-900">Photos Tab</h2>
          ...
          {activeTab === "settings" && (
            <div className="rounded-xl bg-white p-6 shadow-sm">
              <h2 className="text-lg font-bold text-gray-900">Settings Tab</h2>
              <p className="text-gray-500">Coming soon...</p>
            </div>
          )}
        </div>
