...
interface InstitutionPostCardProps {
  post: Post;
  meta: PostMeta;
}

const InstitutionPostCard: React.FC<InstitutionPostCardProps> = ({
  post,
  meta,
}) => {
  ...
  const { mutate: likeMutate, isPending: isLiking } =
    institutionHooks.useToggleLikeInstitutionPost();
  const { mutate: deletePost, isPending: isDeleting } =
    institutionHooks.useDeleteInstitutionPost();
  const { mutate: updatePost, isPending: isUpdating } =
    institutionHooks.useUpdateInstitutionPost();
  const { mutate: toggleBookmark, isPending: isBookmarking } =
    institutionHooks.useToggleBookmarkInstitutionPost();
  ...
  const { mutate: addComment, isPending: isAddingComment } =
    institutionHooks.useAddInstitutionComment({
      postId: post._id,
    });
  const { mutate: deleteComment } =
    institutionHooks.useDeleteInstitutionComment({
      postId: post._id,
    });
...
export default InstitutionPostCard;
