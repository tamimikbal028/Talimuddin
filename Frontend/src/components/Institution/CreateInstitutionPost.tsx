...
const createInstitutionPostSchema = z.object({
  content: z
    .string()
...
type CreateInstitutionPostFormData = z.infer<
  typeof createInstitutionPostSchema
>;

const CreateInstitutionPost: React.FC<{ institutionId: string }> = ({
  institutionId,
}) => {
  const { user } = authHooks.useUser();
  const { mutate: createInstitutionPost, isPending } =
    institutionHooks.useCreateInstitutionPost();
  ...
  const {
    ...
    reset,
    formState: { isValid },
  } = useForm<CreateInstitutionPostFormData>({
    resolver: zodResolver(createInstitutionPostSchema),
    defaultValues: {
      content: "",
      tags: "",
  ...
  const onSubmit = (data: CreateInstitutionPostFormData) => {
...
export default CreateInstitutionPost;
