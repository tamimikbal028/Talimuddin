...
const createDepartmentPostSchema = z.object({
  content: z
    .string()
...
type CreateDepartmentPostFormData = z.infer<typeof createDepartmentPostSchema>;

const CreateDepartmentPost: React.FC<{ DepartmentId: string }> = ({
  DepartmentId,
}) => {
  const { user } = authHooks.useUser();
  const { mutate: createDepartmentPost, isPending } =
    departmentHooks.useCreateDepartmentPost();
  ...
  const {
    ...
    reset,
    formState: { isValid },
  } = useForm<CreateDepartmentPostFormData>({
    resolver: zodResolver(createDepartmentPostSchema),
    defaultValues: {
      content: "",
      tags: "",
  ...
  const onSubmit = (data: CreateDepartmentPostFormData) => {
...
export default CreateDepartmentPost;
