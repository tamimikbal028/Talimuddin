...
const DepartmentPostCard: React.FC<DepartmentPostCardProps> = ({
  post,
  meta,
}) => {
  ...
  const { mutate: likeMutate, isPending: isLiking } =
    departmentHooks.useToggleLikeDepartmentPost();
  const { mutate: deletePost, isPending: isDeleting } =
    departmentHooks.useDeleteDepartmentPost();
  const { mutate: updatePost, isPending: isUpdating } =
    departmentHooks.useUpdateDepartmentPost();
  ...
  const { mutate: addComment, isPending: isAddingComment } =
    departmentHooks.useAddDepartmentComment({
      postId: post._id,
    });
  const { mutate: deleteComment } = departmentHooks.useDeleteDepartmentComment({
    postId: post._id,
  });
